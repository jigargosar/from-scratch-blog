...
postFiles.forEach((file) => {
  ...
  // Determine layout
  let layoutFile = mdParsed.data.layout
    ? path.join(srcDir, mdParsed.data.layout)
    : path.join(srcDir, "post.pug");
  if (!fs.existsSync(layoutFile)) {
    console.warn(
      `Layout file ${layoutFile} not found. Using default post.pug.`
    );
  ...
  console.log(
    `${file} converted to HTML in docs/posts using ${path.basename(
      layoutFile
    )}.`
  );
});
